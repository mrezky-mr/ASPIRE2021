...
#user-defined coefficient of restitution
#the 'sim.coefficient_of_resitution does not work
def cor_bridges(r, v):
	eps = cor						
	return eps
...
N_n = 1000                  # total particles in the sim
rat1 = 0.3                  # mass & number ratio for P1
af1 = 1                     # radius of particle in real system, in m
af2 = 0.1                   # radius of particle in real system, in m
v = 0.1                     # maximum velocity dispersion in m/s, converted to au/yr          

##############################################################################

#calculating derived parameters from the inputs
m_p, r_h, tff, P = sphr(a, M_star, rho_p, r_p)  # sphere properties
v_dp = v * y / au                               # v converted to au/yr
rat2 = 1 - rat1                                 # mass & number ratio for P2, automatic from P1
N_pl1 = rat1 * N_n                              # number of particles in sim (P1)
N_pl2 = rat2 * N_n                              # number of particles in sim (P2)
s_cr1 = np.pi*(af1**2)                          # real collisional xsection of P1, in m2
s_cr2 = np.pi*(af2**2)                          # real collisional xsection of P2, in m2
...
a1, mp1 = parpr(m_pl1, af1, s_cr1, N_pl1)
a2, mp2 = parpr(m_pl2, af2, s_cr2, N_pl2)
sim.softening = max(a1,a2)/au                   # based on the highest a_{c,num}, in au
f = (N_pl1 * a1**3 + N_pl2 * a2**3)/(r_h**3)    # filling factor
...
for i,t in enumerate(times):
    ...
    if i % 3 == 0:
        ...
        ax3.scatter(coords[0][:Na],coords[1][:Na], s=4, label=r"$\sigma_c = {n} \ m^2$".format(n=round(s_cr1,5)))
        ax3.scatter(coords[0][Na:],coords[1][Na:], s=4, label=r"$\sigma_c = {n} \ m^2$".format(n=round(s_cr2,5)))
        ...
        ax4.scatter3D(coords[0][:Na],coords[1][:Na],coords[2][:Na], s=10, label=r"$\sigma_c = {n} m^2$".format(n=round(s_cr1,5)))
        ax4.scatter3D(coords[0][Na:],coords[1][Na:],coords[2][Na:], s=10, label=r"$\sigma_c = {n} m^2$".format(n=round(s_cr2,5)))
        ax4.set_xlabel('X (au)')
        ax4.set_ylabel('Y (au)')
        ax4.set_zlabel('Z (au)')
...
ax6.scatter(coords[0][:Na],coords[1][:Na], s=4, label=r"$\sigma_c = {n} \ \text{m}^2$".format(n=round(s_cr1,5)))
ax6.scatter(coords[0][Na:],coords[1][Na:], s=4, label=r"$\sigma_c = {n} \ \text{m}^2$".format(n=round(s_cr2,5)))
...
ax7.scatter3D(coords[0][:Na],coords[1][:Na],coords[2][:Na], s=10, label=r"$\sigma_c = {n} \ \text{m}^2$".format(n=round(s_cr1,5)))
ax7.scatter3D(coords[0][Na:],coords[1][Na:],coords[2][Na:], s=10, label=r"$\sigma_c = {n} \ \text{m}^2$".format(n=round(s_cr2,5)))
ax7.set_xlabel('X (au)')
ax7.set_ylabel('Y (au)')
ax7.set_zlabel('Z (au)')
...
file.write("The total amount of particles in the simulation: {a} \n".format(a=N_n))
file.write("Radius of the physical particles (P1): {a} m \n".format(a=af1))
file.write("Radius of the physical particles (P2): {a} m \n".format(a=af2))
...
file.write("Radius of the real P1 particles: {a} m \n".format(a=af1))
file.write("Collisional cross-section of the real P1 particles: {a} m2 \n".format(a=s_cr1))
file.write("Mass ratio for P1: {a} of total mass \n".format(a=rat1))
file.write("Radius of a sim particle (P1): {a} km \n".format(a=a1/1e3))
file.write("Mass of a sim particle (P1): {a} kg \n".format(a=mp1))
file.write("---------- \n")
file.write("The amount of P2 particles in the simulation: {a} \n".format(a=N_pl2))
file.write("Radius of the real P2 particles: {a} m \n".format(a=af2))
file.write("Collisional cross-section of the real P2 particles: {a} m2 \n".format(a=s_cr2))
file.write("Mass ratio for P2: {a} of total mass \n".format(a=rat2))
file.write("Radius of a sim particle (P2): {a} km \n".format(a=a2/1e3))
file.write("Mass of a sim particle (P2): {a} kg \n".format(a=mp2))
file.close()